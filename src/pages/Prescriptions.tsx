  ...
  return (
    <div className="space-y-6 animate-slide-up max-w-4xl mx-auto">
      <div>
        ...
        <p className="text-muted-foreground mt-1">Defina o plano nutricional diário do paciente.</p>
      </div>
              ...
              <Select value={selectedPatient} onValueChange={setSelectedPatient}>
                ...
                <SelectContent>
                  {patients.filter(p => p.active).map(p => (
                    <SelectItem key={p.id} value={p.id}>{p.name} - Leito {p.bed}</SelectItem>
                  ))}
                </SelectContent>
              </Select>
            </div>
            ...
            {patient && (
              <div className="p-4 bg-slate-50 rounded-lg border flex flex-col sm:flex-row gap-4 items-start sm:items-center justify-between">
                <div>
                  ...
                  <p className="text-sm text-muted-foreground">{patient.ward} - Leito {patient.bed} | Reg: {patient.recordId}</p>
                </div>
      ...
      <Card className="shadow-md">
        <CardHeader className="bg-slate-50/50 rounded-t-xl border-b">
          ...
          <CardDescription>Defina o que será preparado no lactário.</CardDescription>
        </CardHeader>
        <CardContent className="pt-6">
          <Tabs value={type} onValueChange={(v) => setType(v as 'milk' | 'meal')}>
            <TabsList className="grid w-full grid-cols-2 mb-6">
              <TabsTrigger value="milk" className="text-base">Frasco de Leite</TabsTrigger>
              <TabsTrigger value="meal" className="text-base">Refeição (Sólida/Pastosa)</TabsTrigger>
            </TabsList>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div className="space-y-2">
                  ...
                  <Select value={milkType} onValueChange={setMilkType}>
                    ...
                    <SelectContent>
                      <SelectItem value="Fórmula Infantil">Fórmula Infantil Padrão</SelectItem>
                      <SelectItem value="Leite Materno Pasteurizado">Leite Materno Pasteurizado</SelectItem>
                      <SelectItem value="Leite Materno Cru">Leite Materno Cru</SelectItem>
                      <SelectItem value="Fórmula Especial (HA)">Fórmula Especial (HA)</SelectItem>
                    </SelectContent>
                  </Select>
                </div>
                <div className="space-y-2">
                  <Label>Volume (ml) por horário</Label>
                  <Input type="number" value={volume} onChange={e => setVolume(e.target.value)} className="bg-white" />
                </div>
              </div>
            </TabsContent>
            ...
            <TabsContent value="meal" className="space-y-4">
              <div className="space-y-2">
                ...
                <Input 
                  ...
                  className="bg-white"
                />
                <p className="text-xs text-muted-foreground">Esta descrição será impressa diretamente na etiqueta.</p>
              </div>
            </TabsContent>
            ...
            <div className="mt-8 space-y-2 pt-6 border-t">
              ...
              />
              <p className="text-xs text-muted-foreground">Separe os horários por vírgula. Uma etiqueta será gerada para cada horário na impressão em lote.</p>
              
              <div className="flex flex-wrap gap-2 mt-3">
                {times.split(',').filter(t => t.trim()).map((t, i) => (
                  <Badge key={i} variant="secondary" className="px-3 py-1 text-sm">{t.trim()}</Badge>
                ))}
              </div>
            </div>
          </TabsContent>
      ...
      <div className="flex justify-end gap-4">
        ...
        <Button onClick={handleSave} className="px-8 font-semibold">Salvar Prescrição</Button>
      </div>
    </div>
  )
}
